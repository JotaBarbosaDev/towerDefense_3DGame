...
namespace Core.Data
{
		...
		where TGameManager : GameManagerBase<TGameManager, TDataStore>
	{
		...
		public virtual void SetVolumes(float master, float sfx, float music, bool save)
		{
			...
			// Transform 0-1 into logarithmic -80-0 when a mixer is available.
			if (gameMixer != null && !string.IsNullOrEmpty(masterVolumeParameter))
			{
				gameMixer.SetFloat(masterVolumeParameter, LogarithmicDbTransform(Mathf.Clamp01(master)));
			}
			if (gameMixer != null && !string.IsNullOrEmpty(sfxVolumeParameter))
			{
				gameMixer.SetFloat(sfxVolumeParameter, LogarithmicDbTransform(Mathf.Clamp01(sfx)));
			}
			if (gameMixer != null && !string.IsNullOrEmpty(musicVolumeParameter))
			{
		...
		protected void EnsureDataStore()
		{
			if (m_DataStore != null)
			{
				return;
